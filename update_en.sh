...
if [ "${publicFileMd5}" != "${md5check}"  ]; then
	wget -O Tpublic.sh http://bt.sh.cn/src/public.sh -T 20;
...
wget -T 5 -O /tmp/panel.zip http://bt.sh.cn/src/LinuxPanel-7.7.0.zip
...
if [ "${check_bt}" = "" ];then
	rm -f /etc/init.d/bt
	wget -O /etc/init.d/bt http://bt.sh.cn/src/bt6.init -T 20
